...
def create_admin_routes(ctx):
    ...
    @bp.get('/fanzone/stats/<fixture_id>')
    def fanzone_stats(fixture_id):
        base = ctx.get("BASE_DIR", "")

        # ---- Load votes ----
        votes_path = _runtime_path(ctx, "fan_votes.json")
        votes_blob = _read_json(votes_path, {"fixtures": {}})
        fixtures = votes_blob.get("fixtures") if isinstance(votes_blob, dict) else {}

        fx = fixtures.get(fixture_id, {}) if isinstance(fixtures, dict) else {}
        if not isinstance(fx, dict):
            fx = {}
        ...
        if total > 0:
            home_pct = round((home_votes / total) * 100)
        ...
        winner_rec = winners_blob.get(fixture_id) if isinstance(winners_blob, dict) else {}

        if not isinstance(winner_rec, dict):
            winner_rec = {}

        winner = (
                winner_rec.get("winner")
                or winner_rec.get("winner_side")
                or ""
        )

        last_choice = ""
        user = session.get(USER_SESSION_KEY)
        if user and isinstance(fx.get("voters"), dict):
            uid = str(user.get("discord_id") or "")
            if uid:
                last_choice = str(fx["voters"].get(uid) or "").lower()
