...
window.loadSplits = loadSplits;
    ...
    async function loadBackups(){
      try{
        ...
        settingsWrap.className = 'backup-settings';
        settingsWrap.innerHTML = `
          <div class="backup-settings-row">
            <label class="backup-settings-toggle">
              <input type="checkbox" id="backup-auto-enabled" />
              <span>Auto backup JSON every</span>
            </label>
            <div class="backup-settings-interval">
              <input type="number" id="backup-auto-hours" min="0.5" step="0.5" />
              <span class="backup-settings-units">hours</span>
            </div>
          </div>
          <div class="backup-settings-meta">
            <span id="backup-last-run"></span>
            <span id="backup-auto-status"></span>
          </div>
        `;
        w.insertBefore(settingsWrap, s);

        const autoEnabledInput = settingsWrap.querySelector('#backup-auto-enabled');
        const autoHoursInput = settingsWrap.querySelector('#backup-auto-hours');
        const lastRunLabel = settingsWrap.querySelector('#backup-last-run');
        const statusLabel = settingsWrap.querySelector('#backup-auto-status');

        const intervalSeconds = Number(autoBackup.interval_seconds ?? (4 * 60 * 60));
        const intervalHoursRaw = intervalSeconds / 3600;
        const intervalHours = Math.round(intervalHoursRaw * 100) / 100;

        const syncAutoBackupUI = (settings) => {
          const enabled = settings?.enabled ?? true;
          const intervalSecondsValue = Number(settings?.interval_seconds ?? intervalSeconds);
          const intervalHoursValue = Math.round((intervalSecondsValue / 3600) * 100) / 100;
          autoEnabledInput.checked = !!enabled;
          autoHoursInput.value = Number.isFinite(intervalHoursValue) ? intervalHoursValue : 4;
          const lastTs = settings?.last_backup_ts ?? autoBackup.last_backup_ts;
          if (lastTs) {
            lastRunLabel.textContent = `Last backup: ${new Date(lastTs * 1000).toLocaleString()}`;
          } else {
            lastRunLabel.textContent = 'Last backup: —';
          }
          statusLabel.textContent = enabled ? 'Auto backups enabled' : 'Auto backups disabled';
        };

        syncAutoBackupUI(autoBackup);

        const saveAutoBackupSettings = async (payload) => {
          try {
        ...
        autoEnabledInput.addEventListener('change', () => {
          saveAutoBackupSettings({ enabled: autoEnabledInput.checked });
        });

        autoHoursInput.addEventListener('change', () => {
          const hours = Number(autoHoursInput.value);
          if (!Number.isFinite(hours) || hours <= 0) {
            notify('Enter a valid number of hours.', false);
            return;
          }
          saveAutoBackupSettings({ interval_seconds: Math.round(hours * 3600) });
        });
